...
自注意力（Self-Attnetion）相比LSTM的优势在于：

- 

- LSTM因为采用的是单调递归的运算方式，在处理长序列时容易面临梯度消失或梯度爆炸的问题，而注意力机制能够选择性地聚焦于序列中的特定部分，使网络能更好地捕捉时间序列数据中的长期依赖关系
- 注意力机制使得模型可以解释其预测过程中的决策依据，通过可视化注意力权重，可以得知模型关注的是输入的特征的哪些部分
- LSTM需要储存和更新大量的记忆状态，而注意力机制只需要计算注意力权重和加权和操作，有些情况下能够使训练更有效率
