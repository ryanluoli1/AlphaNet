...
为了模拟真实的时间序列预测场景，我们采用滚动训练法，让模型在时间轴上每隔半年滚动训练一次：

<center>
<img src="Images/滚动训练.png" width="750" align="center"/>
</center>

每次在1500个交易日的样本内数据上进行训练和验证，训练集和验证集的比例为4:1，验证集的主要目的是用来判断模型最佳的迭代次数（是否需要早停）。样本外的测试集为紧跟着样本内数据的后126个交易日（半年）。如图，下一次训练时，将整个样本内/外数据窗口在时间轴上向右移动126个交易日。
